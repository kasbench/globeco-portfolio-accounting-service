...
# Deploy main app resources for GlobeCo Portfolio Accounting Service (excluding Hazelcast and Postgres)
set -euo pipefail
...
deploy_app() {
    print_info "Deploying main app resources to namespace $NAMESPACE"
    apply_resource "$DEPLOYMENTS_DIR/namespace.yaml"
    apply_resource "$DEPLOYMENTS_DIR/configmap.yaml"
    apply_resource "$DEPLOYMENTS_DIR/secrets.yaml"
    apply_resource "$DEPLOYMENTS_DIR/deployment.yaml"
    apply_resource "$DEPLOYMENTS_DIR/service.yaml"
    apply_resource "$DEPLOYMENTS_DIR/ingress.yaml"
    # apply_resource "$DEPLOYMENTS_DIR/hpa.yaml"
    apply_resource "$DEPLOYMENTS_DIR/network-policy.yaml"
...
show_status() {
    kubectl get deployments,svc,ingress,hpa,pdb,configmap,secret -n "$NAMESPACE" | grep -v hazelcast | grep -v postgresql || true
}
