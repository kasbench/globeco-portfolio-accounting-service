...
# Deploy Hazelcast resources for GlobeCo Portfolio Accounting Service
set -euo pipefail
...
deploy_hazelcast() {
    print_info "Deploying Hazelcast resources to namespace $NAMESPACE"
    apply_resource "$DEPLOYMENTS_DIR/hazelcast.yaml"
    # Apply Hazelcast HPA if present
    if grep -q 'kind: HorizontalPodAutoscaler' "$DEPLOYMENTS_DIR/hpa.yaml"; then
        kubectl apply -f <(awk '/kind: HorizontalPodAutoscaler/{p=1} p; /name: hazelcast-hpa/{exit}' "$DEPLOYMENTS_DIR/hpa.yaml") -n "$NAMESPACE" || true
        print_success "Hazelcast HPA applied"
    fi
    # Apply Hazelcast services if present
    if grep -q 'name: hazelcast-service' "$DEPLOYMENTS_DIR/hazelcast.yaml"; then
        print_info "Hazelcast services already in hazelcast.yaml"
    fi
    print_success "Hazelcast deployment complete"
}
...
show_status() {
    kubectl get statefulsets,svc,pdb,hpa -n "$NAMESPACE" | grep hazelcast || true
}

show_logs() {
    local pods=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/component=cache -o name)
    for pod in $pods; do
        echo "=== Logs from $pod ==="
...
main() {
    ...
    case "$COMMAND" in
        deploy) deploy_hazelcast;;
        destroy) destroy_hazelcast;;
        status) show_status;;
        logs) show_logs;;
